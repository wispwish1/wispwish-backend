...
// Initialize Stripe with dummy key for testing if not configured
let stripe;
try {
  if (!process.env.STRIPE_SECRET_KEY || process.env.STRIPE_SECRET_KEY === 'sk_test_dummy_key_for_testing') {
    console.warn('⚠️ STRIPE_SECRET_KEY is not configured - payment processing will not work');
    stripe = null;
  } else {
...
router.post('/create-checkout-session', async (req, res) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: `https://www.wispwish.com/giftgenerator.html?session_id={CHECKOUT_SESSION_ID}&gift_id=${giftId}&payment_success=true`,
      cancel_url: 'https://www.wispwish.com/giftgenerator.html?payment_cancelled=true',
...
async function processPaymentCompletion(giftId, stripeSessionId, buyerEmail, status = PAYMENT_STATUSES.COMPLETE) {
  ...
  try {
    ...
    if (gift.deliveryMethod === 'email' && gift.deliveryEmail && status === PAYMENT_STATUSES.COMPLETE) {
      ...
      if (scheduledDate <= now) {
        // Handle WishKnot gifts differently
        if (updatedGift.giftType === 'wishknot') {
          // Import WishKnot model dynamically
          const WishKnot = (await import('../models/WishKnot.js')).default;
          ...
          if (wishKnot) {
            console.log('🪢 Sending WishKnot email with access token:', wishKnot.accessToken);
            const wishKnotEmailResult = await nodemailerService.sendWishKnotEmail({
              recipientEmail: updatedGift.deliveryEmail,
              recipientName: updatedGift.recipientName,
              senderName: updatedGift.senderName,
              knotType: wishKnot.knotType,
              occasion: updatedGift.occasion,
            ...
            if (wishKnotEmailResult.success) {
              ...
              console.log(`✅ WishKnot delivered immediately: ${updatedGift._id}`);
            } else {
              console.error('❌ Failed to send WishKnot email after payment:', wishKnotEmailResult.error);
            ...
            console.error('❌ WishKnot record not found for gift:', updatedGift._id);
            updatedGift.deliveryStatus = 'failed';
            await updatedGift.save();
          }
        } else {
          // Handle regular gifts (non-WishKnot)
          try {
            // For combo gifts, ensure we pass the properly structured content
            const giftData = {
        ...
        updatedGift.deliveryStatus = 'scheduled';
        await updatedGift.save();
        console.log(`📅 Gift scheduled for delivery: ${updatedGift._id}`);
      }
    }
